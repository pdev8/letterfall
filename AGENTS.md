...
- No test runner or linter yet (PL-100/PL-102 in the roadmap add Jest/ESLint).
...
- **Tests ship with the work.** Every PR that changes game logic, scoring,
  ...
  PL-100 lands Jest, attach `npx tsx` test-script evidence to the PR.)
- Branches: `pl-<ticket>/<slug>` for roadmap tickets, `docs/<slug>` for
  doc-only changes.
- **PR titles encode merge order:** `[<n>. <scope>] <title>`, where `n` is a
  ...
  (`[5. E0] PL-100: Jest test suite`). Stacked PRs append `(after #N)` and
  set their GitHub base to the PR they depend on.
- `docs/ROADMAP.md` is the canonical plan (epics PL-1xx tickets + specs for
...
- **`src/game.ts`** — the entire rule set as a pure reducer
  ...
  difficulty knobs on `state.config` (PL-131; bays designated per PL-179).
...
- **UI** — `App.tsx` is a small screen router (SafeAreaProvider + StatusBar)
  ...
  (PL-168). `src/screens/HomeScreen.tsx` is the landing menu (Free Play / Daily)
...
- Parked cards are playable but **never count toward the win**:
  ...
  PL-179 (supersedes PL-177's "any empty column" rule).
